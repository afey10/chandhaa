...
interface FormState {
  registrationNumber: string; categoryId: string; make: string; model: string; year: string; colour: string;
  engineNumber: string; chassisNumber: string; ownerFullName: string; ownerIdCard: string; ownerAddress: string;
  contactNumber: string; annualFeeExpiry: string; insuranceExpiry: string; roadworthinessExpiry: string; remarks: string;
}

const EMPTY: FormState = {
  registrationNumber: "", categoryId: "", make: "", model: "", year: "", colour: "", engineNumber: "", chassisNumber: "",
  ownerFullName: "", ownerIdCard: "", ownerAddress: "", contactNumber: "", annualFeeExpiry: "", insuranceExpiry: "",
  roadworthinessExpiry: "", remarks: "",
};
...
export default function VehicleFormPage() {
  ...
  useEffect(() => {
    ...
    rpc
      ...
      .then((res) => {
        const v = res.vehicle;
        setForm({
          registrationNumber: v.registrationNumber, categoryId: v.categoryId, make: v.make || "", model: v.model || "",
          year: v.year?.toString() || "", colour: v.colour || "", engineNumber: v.engineNumber || "", chassisNumber: v.chassisNumber || "",
          ownerFullName: v.ownerFullName, ownerIdCard: v.ownerIdCard, ownerAddress: v.ownerAddress, contactNumber: v.contactNumber || "",
          annualFeeExpiry: v.annualFeeExpiry, insuranceExpiry: v.insuranceExpiry, roadworthinessExpiry: v.roadworthinessExpiry,
          remarks: v.remarks || "",
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (photo) photograph = await rpc.uploadFile(photo, "vehicles");

      const payload = { ...form, photograph };

      if (isEdit) {
        await rpc.updateVehicle(id!, payload);
        showToast(`Vehicle ${form.registrationNumber} has been updated.`);
        navigate(`/vehicles/${id}`);
      } else {
        const res = await rpc.createVehicle(payload);
        showToast(`Vehicle ${form.registrationNumber} has been successfully added.`);
  ...
  if (loading) {
    return <AppLayout><div className="py-20 text-center text-gray-400">Loading vehicle…</div></AppLayout>;
  }

  return (
    <AppLayout>
      <div className="mx-auto max-w-3xl">
        <h1 className="font-display text-2xl font-semibold text-navy-950">{isEdit ? "Edit Vehicle" : "Add New Vehicle"}</h1>
        <p className="mt-1 text-sm text-gray-500">Fields marked with an asterisk are required.</p>

        <form onSubmit={handleSubmit} className="mt-6 space-y-6">
          <section className="card p-5">
            <h2 className="mb-4 font-display text-base font-semibold text-navy-950">Vehicle Information</h2>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field label="Registration Number" required>
                <input value={form.registrationNumber} onChange={(e) => update("registrationNumber", e.target.value.toUpperCase())} className={inputClass} placeholder="e.g. TV-1234" />
              ...
              <Field label="Make"><input value={form.make} onChange={(e) => update("make", e.target.value)} className={inputClass} /></Field>
              <Field label="Model"><input value={form.model} onChange={(e) => update("model", e.target.value)} className={inputClass} /></Field>
          ...
          <section className="card p-5">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field label="Chassis Number"><input value={form.chassisNumber} onChange={(e) => update("chassisNumber", e.target.value)} className={inputClass} /></Field>
              ...
              <div className="sm:col-span-2">
                <label className="mb-1 block text-sm font-medium text-gray-700">Vehicle Photograph</label>
                {existingPhoto && !photo && (
                  <img src={publicFileUrl(existingPhoto) || ""} alt="Current vehicle" className="mb-2 h-28 w-28 rounded-lg object-cover" />
                )}

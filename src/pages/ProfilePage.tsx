...
export default function ProfilePage() {
  ...
  useEffect(() => {
    rpc.getProfile().then(setProfile);
  }, []);
  ...
  const handlePictureChange = async (file: File | null) => {
    ...
    try {
      const path = await rpc.uploadFile(file, "profiles");
      await rpc.updateProfilePicture(path);
      setProfile((p: any) => ({ ...p, profilePicture: path }));
      await refresh();
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    setSavingPw(true);
    try {
      await rpc.changePassword(currentPassword, newPassword);
        ...
        <div className="card p-6 text-center lg:col-span-1">
          <div className="relative mx-auto w-fit">
            {profile?.profilePicture ? (
              <img src={publicFileUrl(profile.profilePicture) || ""} className="h-24 w-24 rounded-full object-cover" alt="" />
            ) : (
          ...
          <span className="mt-2 inline-block rounded-full bg-navy-50 px-3 py-1 text-xs font-medium capitalize text-navy-700">{user?.role}</span>
        </div>
        ...
        <div className="card p-6 lg:col-span-2">
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            <div>
              ...
              <input type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm outline-none focus:border-navy-600 focus:ring-2 focus:ring-navy-100" />
              {pwErrors.currentPassword && <p className="mt-1 text-xs text-red-600">{pwErrors.currentPassword}</p>}
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">New Password</label>
              <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm outline-none focus:border-navy-600 focus:ring-2 focus:ring-navy-100" />
              {pwErrors.newPassword && <p className="mt-1 text-xs text-red-600">{pwErrors.newPassword}</p>}
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">Confirm New Password</label>
              <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm outline-none focus:border-navy-600 focus:ring-2 focus:ring-navy-100" />
              {pwErrors.confirmPassword && <p className="mt-1 text-xs text-red-600">{pwErrors.confirmPassword}</p>}
            </div>
            <button type="submit" disabled={savingPw} className="rounded-lg bg-navy-900 px-5 py-2.5 text-sm font-semibold text-white hover:bg-navy-800 disabled:opacity-60">

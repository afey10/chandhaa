...
export default function VesselListPage() {
  ...
  const fetchList = useCallback(() => {
    setLoading(true);
    rpc
      .listVessels({ search: debouncedSearch, category, overallStatus, sortBy, sortDir, page, pageSize: PAGE_SIZE })
      .then((res: any) => {
        setItems(res.items);
        setTotal(res.total);
  ...
  const handleDelete = async () => {
    ...
    try {
      await rpc.deleteVessel(deleteTarget.id);
    ...
    <AppLayout>
      ...
      <div className="card hidden overflow-x-auto sm:block">
        <table className="w-full min-w-[900px] text-sm">
          ...
          <tbody>
            {loading && <tr><td colSpan={8} className="px-4 py-10 text-center text-gray-400">Loading vessels…</td></tr>}
            {!loading && items.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-10 text-center text-gray-400">No vessels match your search or filters.</td></tr>
            )}
            {!loading &&
              items.map((v) => (
                <tr key={v.id} className="border-b border-[#f0f1f5] hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-1">
                      <Link to={`/vessels/${v.id}`} className="rounded-lg p-2 text-gray-500 hover:bg-gray-100" title="View"><Eye size={16} /></Link>
                      {canEdit && <Link to={`/vessels/${v.id}/edit`} className="rounded-lg p-2 text-gray-500 hover:bg-gray-100" title="Edit"><Pencil size={16} /></Link>}
                      {canDelete && (
      ...
      <div className="space-y-3 sm:hidden">
        {loading && <div className="card px-4 py-10 text-center text-gray-400">Loading vessels…</div>}
        {!loading && items.length === 0 && <div className="card px-4 py-10 text-center text-gray-400">No vessels match your search or filters.</div>}
        {!loading &&
          items.map((v) => (
            <div key={v.id} className="card p-4">
              <div className="flex items-start justify-between">
                <button onClick={() => navigate(`/vessels/${v.id}`)} className="font-mono-reg text-base font-semibold text-navy-900">
                  {v.registrationNumber}
                </button>
                <div className="flex gap-1">
                  <Link to={`/vessels/${v.id}`} className="rounded-lg p-1.5 text-gray-500"><Eye size={16} /></Link>
                  {canEdit && <Link to={`/vessels/${v.id}/edit`} className="rounded-lg p-1.5 text-gray-500"><Pencil size={16} /></Link>}
                  {canDelete && <button onClick={() => setDeleteTarget(v)} className="rounded-lg p-1.5 text-gray-500"><Trash2 size={16} /></button>}
                </div>
              </div>

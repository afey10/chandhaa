...
interface UserRow {
  id: string; serviceNumber: string; fullName: string; role: "admin" | "staff";
  canAddRecords: boolean; canEditRecords: boolean; isActive: boolean;
}
...
export default function SettingsPage() {
  ...
  const load = () => {
    setLoading(true);
    rpc.listUsers().then((items: any) => setUsers(items)).catch((err) => showToast(getErrorMessage(err), "error")).finally(() => setLoading(false));
  };
  ...
  const handlePermissionToggle = async (u: UserRow, field: "canAddRecords" | "canEditRecords" | "isActive") => {
    try {
      await rpc.updateUserPermissions(u.id, { [field]: !u[field] });
      load();
    } catch (err) {
      showToast(getErrorMessage(err), "error");
    }
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      await rpc.adminResetPassword(resetTarget.id, resetPassword);
  ...
  return (
    <AppLayout>
      <div className="mb-5 flex items-center justify-between">
        ...
        <button onClick={() => setShowAddUser((v) => !v)} className="flex items-center gap-2 rounded-lg bg-navy-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-navy-800">
      ...
      {showAddUser && (
        <form onSubmit={handleCreateUser} className="card mb-5 grid grid-cols-1 gap-4 p-5 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <button type="submit" className="rounded-lg bg-navy-900 px-5 py-2.5 text-sm font-semibold text-white hover:bg-navy-800">Create Account</button>
          </div>
        </form>
      )}
      ...
      <div className="card hidden overflow-x-auto sm:block">
        <table className="w-full min-w-[700px] text-sm">
          ...
          <tbody>
            {loading && <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">Loading users…</td></tr>}
            {!loading &&
              users.map((u) => (
                <tr key={u.id} className="border-b border-[#f0f1f5]">
                  ...
                  <td className="px-4 py-3"><input type="checkbox" disabled={u.role === "admin"} checked={u.canAddRecords || u.role === "admin"} onChange={() => handlePermissionToggle(u, "canAddRecords")} /></td>
                  <td className="px-4 py-3"><input type="checkbox" disabled={u.role === "admin"} checked={u.canEditRecords || u.role === "admin"} onChange={() => handlePermissionToggle(u, "canEditRecords")} /></td>
                  <td className="px-4 py-3"><input type="checkbox" checked={u.isActive} onChange={() => handlePermissionToggle(u, "isActive")} /></td>
      ...
      <div className="space-y-3 sm:hidden">
        {loading && <div className="card px-4 py-8 text-center text-gray-400">Loading users…</div>}
        {!loading &&
          users.map((u) => (
            <div key={u.id} className="card p-4">
              <div className="flex items-start justify-between">
                <div>
                  <div className="font-medium text-navy-950">{u.fullName}</div>
                  <div className="font-mono-reg text-xs text-gray-500">{u.serviceNumber} · {u.role}</div>
                </div>
                <button onClick={() => setResetTarget(u)} className="text-sm font-medium text-navy-700">Reset</button>
              </div>
              <div className="mt-3 flex flex-wrap gap-4 text-sm text-gray-600">
                <label className="flex items-center gap-1.5"><input type="checkbox" disabled={u.role === "admin"} checked={u.canAddRecords || u.role === "admin"} onChange={() => handlePermissionToggle(u, "canAddRecords")} /> Add</label>
                <label className="flex items-center gap-1.5"><input type="checkbox" disabled={u.role === "admin"} checked={u.canEditRecords || u.role === "admin"} onChange={() => handlePermissionToggle(u, "canEditRecords")} /> Edit</label>
                <label className="flex items-center gap-1.5"><input type="checkbox" checked={u.isActive} onChange={() => handlePermissionToggle(u, "isActive")} /> Active</label>
              </div>
            </div>
          ))}
      </div>
      ...
      {resetTarget && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
          <form onSubmit={handleResetPassword} className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl">
            ...
            <input type="text" minLength={8} required value={resetPassword} onChange={(e) => setResetPassword(e.target.value)} placeholder="New password (min. 8 characters)" className={`${inputClass} mt-4`} />
            <div className="mt-5 flex justify-end gap-2">
              <button type="button" onClick={() => { setResetTarget(null); setResetPassword(""); }} className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</button>
              <button type="submit" className="rounded-lg bg-navy-900 px-4 py-2 text-sm font-medium text-white hover:bg-navy-800">Reset Password</button>
            </div>
          </form>
        </div>
      )}

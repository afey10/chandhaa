...
const SELECT_WITH_CATEGORY = "*, vehicle_categories(name)";
...
function enrich(row: any, warningDays: number) {
  return {
    ...
    categoryName: row.vehicle_categories?.name || row.category_name,
    make: row.make,
    model: row.model,
    year: row.year,
    colour: row.colour,
...
export async function getDashboard(_req: Request, res: Response) {
  ...
  const { data: rows, error } = await supabase.from("vehicles").select("annual_fee_expiry, insurance_expiry, roadworthiness_expiry");
  ...
  for (const c of categories || []) {
    const { count } = await supabase.from("vehicles").select("*", { count: "exact", head: true }).eq("category_id", c.id);
...
export async function getExpiringList(req: Request, res: Response) {
  ...
  const { data: rows, error } = await supabase.from("vehicles").select(SELECT_WITH_CATEGORY).order(column, { ascending: true });
  if (error) throw error;
...
export async function list(req: Request, res: Response) {
  ...
  const { data: allRows, error } = await supabase.from("vehicles").select(SELECT_WITH_CATEGORY);
  if (error) throw error;
  ...
  const sortMap: Record<string, (r: any) => string> = {
    ...
    category: (r) => r.vehicle_categories?.name || "",
  };
  ...
  const { count: totalAll } = await supabase.from("vehicles").select("*", { count: "exact", head: true });
...
export async function getById(req: Request, res: Response) {
  const warningDays = getWarningDays();
  const { data: row, error } = await supabase.from("vehicles").select(SELECT_WITH_CATEGORY).eq("id", req.params.id).maybeSingle();
  if (error) throw error;
  ...
  res.json({ vehicle: enrich(row, warningDays), documents: documents || [] });
}
...
export async function create(req: Request, res: Response) {
  ...
  if (error) {
    if (error.code === "23505") return res.status(409).json({ error: "A vehicle with this registration number already exists." });
    throw error;
  }
  ...
  res.status(201).json({ id: inserted.id, message: `Vehicle ${data.registrationNumber} has been successfully added.` });
}

export async function update(req: Request, res: Response) {
  const data = vehicleSchema.parse(req.body);
  const { data: existing } = await supabase.from("vehicles").select("photograph").eq("id", req.params.id).maybeSingle();
  ...
  await logAudit({
    user: req.user!,
    action: "Edited vehicle",
    recordType: "vehicle",
    recordId: req.params.id,
    recordLabel: data.registrationNumber,
  });

  res.json({ message: `Vehicle ${data.registrationNumber} has been updated.` });
}

export async function remove(req: Request, res: Response) {
  const { data: existing } = await supabase.from("vehicles").select("registration_number").eq("id", req.params.id).maybeSingle();
  if (!existing) return res.status(404).json({ error: "Vehicle not found." });
  ...
  res.json({ message: `Vehicle ${existing.registration_number} has been deleted.` });
}

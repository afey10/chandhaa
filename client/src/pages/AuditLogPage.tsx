...
export default function AuditLogPage() {
  ...
  const fetchLog = useCallback(() => {
    setLoading(true);
    api
      .get("/audit-log", { params: { page, pageSize: PAGE_SIZE, search, recordType } })
      .then((res) => {
    ...
    <AppLayout>
              ...
              <div>
                <span className="text-sm text-navy-950">
                  <span className="font-medium">
                    {entry.userName} ({entry.serviceNumber})
                  </span>{" "}
                  {entry.action.toLowerCase()}
                  {entry.recordLabel ? (
                    <>
                      {" "}
                      <span className="font-mono-reg font-medium">{entry.recordLabel}</span>
                    </>
                  ) : null}
                  .
                </span>
      ...
      <div className="mt-4 flex items-center justify-between text-sm text-gray-500">
        ...
        <div className="flex gap-2">
          ...
          <button disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)} className="rounded-lg border border-gray-300 px-3 py-1.5 disabled:opacity-40">
            Next
          </button>
        </div>
      </div>

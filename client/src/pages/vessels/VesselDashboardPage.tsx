...
import { Anchor, Wallet, ShieldAlert, Wrench, ChevronRight } from "lucide-react";
...
export default function VesselDashboardPage() {
  ...
  useEffect(() => {
    rpc.getVesselDashboard().then(setData).catch((err) => showToast(getErrorMessage(err), "error"));
  }, []);

  useEffect(() => {
    setLoadingList(true);
    rpc
      .getVesselsExpiring(activeTab)
  ...
  return (
    <AppLayout>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-semibold text-navy-950">Vessel Dashboard</h1>
          <p className="mt-1 text-sm text-gray-500">Overview of all registered vessels at Tha. Veymandoo.</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Total Vessels" value={data?.totalVessels ?? "—"} icon={<Anchor size={18} />} />
      ...
      <div className="card mt-6">
        ...
        <div className="hidden overflow-x-auto sm:block">
          <table className="w-full min-w-[640px] text-sm">
            ...
            <tbody>
              {loadingList && <tr><td colSpan={6} className="px-4 py-6 text-center text-gray-400">Loading…</td></tr>}
              {!loadingList && items.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">No vessels are currently in this expiry window.</td></tr>
              )}
              {!loadingList &&
                items.map((v) => {
                  ...
                  return (
                    <tr key={v.id} className="border-b border-[#f0f1f5] hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <ReminderButton type="vessel" registrationNumber={v.registrationNumber} contactNumber={v.contactNumber} status={v.status} compact />
                      </td>
                      <td className="px-4 py-3 text-right">
                        <Link to={`/vessels/${v.id}`} className="inline-flex items-center gap-1 text-sm font-medium text-navy-700 hover:underline">
        ...
        <div className="divide-y divide-[#f0f1f5] sm:hidden">
          ...
          {!loadingList && items.length === 0 && (
            <div className="px-4 py-8 text-center text-gray-400">No vessels are currently in this expiry window.</div>
          )}
          {!loadingList &&
            items.map((v) => {
              const expiry = (v as any)[tabConfig.dateField];
              ...
              return (
                <div key={v.id} className="flex items-center justify-between gap-2 px-4 py-3">
                  <Link to={`/vessels/${v.id}`} className="min-w-0 flex-1">

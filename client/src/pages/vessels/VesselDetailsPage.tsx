...
export default function VesselDetailsPage() {
  ...
  if (loading || !vessel) {
    return (
      <AppLayout>
        <div className="py-20 text-center text-gray-400">Loading vessel details…</div>
      </AppLayout>
    );
  }
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-sm text-gray-500">
            {vessel.categoryName} {vessel.vesselName ? `· ${vessel.vesselName}` : ""}
          </div>
        </div>
          ...
          {documents.length > 0 && (
            <div className="card p-5">
              ...
              <div className="space-y-2">
                {documents.map((d) => (
                  <a key={d.id} href={`/${d.file_path}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm text-navy-700 hover:bg-gray-50">
        ...
        <div className="space-y-5">
          {vessel.photograph && (
            <div className="card overflow-hidden p-0">
              <img src={`/${vessel.photograph}`} alt="Vessel" className="h-48 w-full object-cover" />
            </div>
          )}
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-1.5">
                  <StatusBadge status={vessel.status.annualFee} daysRemaining={daysRemaining(vessel.annualFeeExpiry)} />
                </div>
              </div>
              <div>
                ...
                <div className="mt-1.5">
                  <StatusBadge status={vessel.status.insurance} daysRemaining={daysRemaining(vessel.insuranceExpiry)} />
                </div>
              </div>
              <div>
                ...
                <div className="mt-1.5">
                  <StatusBadge status={vessel.status.roadworthiness} daysRemaining={daysRemaining(vessel.roadworthinessExpiry)} />
                </div>
              </div>
            </div>
          </div>

...
export default function VesselFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        await api.put(`/vessels/${id}`, fd, { headers: { "Content-Type": "multipart/form-data" } });
        showToast(`Vessel ${form.registrationNumber} has been updated.`);
        navigate(`/vessels/${id}`);
      } else {
        const res = await api.post(`/vessels`, fd, { headers: { "Content-Type": "multipart/form-data" } });
        showToast(`Vessel ${form.registrationNumber} has been successfully added.`);
        navigate(`/vessels/${res.data.id}`);
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <AppLayout>
        <div className="py-20 text-center text-gray-400">Loading vessel…</div>
      </AppLayout>
    );
  }
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-6">
          <section className="card p-5">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field label="Make / Builder">
                <input value={form.builder} onChange={(e) => update("builder", e.target.value)} className={inputClass} />
              </Field>
          ...
          <section className="card p-5">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field label="Hull Number">
                <input value={form.hullNumber} onChange={(e) => update("hullNumber", e.target.value)} className={inputClass} />
              </Field>
              <Field label="Length (m)">
                <input type="number" step="0.1" value={form.length} onChange={(e) => update("length", e.target.value)} className={inputClass} />
              </Field>
              <Field label="Width (m)">
                <input type="number" step="0.1" value={form.width} onChange={(e) => update("width", e.target.value)} className={inputClass} />
              </Field>
              ...
              <div className="sm:col-span-2">
                ...
                {existingPhoto && !photo && <img src={`/${existingPhoto}`} alt="Current vessel" className="mb-2 h-28 w-28 rounded-lg object-cover" />}

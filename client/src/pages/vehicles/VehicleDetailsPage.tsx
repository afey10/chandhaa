...
export default function VehicleDetailsPage() {
  ...
  if (loading || !vehicle) {
    return (
      <AppLayout>
        <div className="py-20 text-center text-gray-400">Loading vehicle details…</div>
      </AppLayout>
    );
  }
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-sm text-gray-500">
            {vehicle.categoryName} · {vehicle.make} {vehicle.model} {vehicle.year ? `(${vehicle.year})` : ""}
          </div>
        </div>
              ...
              <div className="space-y-2">
                {documents.map((d) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm text-navy-700 hover:bg-gray-50"
                  >
        ...
        <div className="space-y-5">
          {vehicle.photograph && (
            <div className="card overflow-hidden p-0">
              <img src={`/${vehicle.photograph}`} alt="Vehicle" className="h-48 w-full object-cover" />
            </div>
          )}
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-1.5">
                  <StatusBadge status={vehicle.status.annualFee} daysRemaining={daysRemaining(vehicle.annualFeeExpiry)} />
                </div>
              </div>
              <div>
                ...
                <div className="mt-1.5">
                  <StatusBadge status={vehicle.status.insurance} daysRemaining={daysRemaining(vehicle.insuranceExpiry)} />
                </div>
              </div>
              <div>
                ...
                <div className="mt-1.5">
                  <StatusBadge status={vehicle.status.roadworthiness} daysRemaining={daysRemaining(vehicle.roadworthinessExpiry)} />
                </div>
              </div>
            </div>
          </div>

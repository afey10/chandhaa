...
export default function VehicleListPage() {
  ...
  useEffect(() => {
    rpc.listVehicleCategories().then((items: any) => setCategories(items));
  }, []);
  ...
  const fetchList = useCallback(() => {
    setLoading(true);
    rpc
      .listVehicles({ search: debouncedSearch, category, overallStatus, sortBy, sortDir, page, pageSize: PAGE_SIZE })
      .then((res: any) => {
        setItems(res.items);
        setTotal(res.total);
  ...
  const handleDelete = async () => {
    ...
    try {
      await rpc.deleteVehicle(deleteTarget.id);
      showToast(`Vehicle ${deleteTarget.registrationNumber} has been deleted.`);
  ...
  return (
    <AppLayout>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-semibold text-navy-950">Vehicle List</h1>
          <p className="mt-1 text-sm text-gray-500">
            Showing {items.length ? (page - 1) * PAGE_SIZE + 1 : 0}–{Math.min(page * PAGE_SIZE, total)} of {total} vehicles
        ...
        <table className="w-full min-w-[900px] text-sm">
          <thead>
            <tr className="border-b border-[#e5e8f0] text-left text-xs uppercase tracking-wide text-gray-400">
              <th className="cursor-pointer px-4 py-3" onClick={() => toggleSort("registration_number")}>Registration No.</th>
              <th className="cursor-pointer px-4 py-3" onClick={() => toggleSort("owner_name")}>Owner</th>
              <th className="px-4 py-3">ID Card</th>
              <th className="cursor-pointer px-4 py-3" onClick={() => toggleSort("category")}>Category</th>
          ...
          <tbody>
            {loading && <tr><td colSpan={8} className="px-4 py-10 text-center text-gray-400">Loading vehicles…</td></tr>}
            {!loading && items.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-10 text-center text-gray-400">No vehicles match your search or filters.</td></tr>
            )}
            {!loading &&
              items.map((v) => (
                <tr key={v.id} className="border-b border-[#f0f1f5] hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <button onClick={() => navigate(`/vehicles/${v.id}`)} className="font-mono-reg font-medium text-navy-900 hover:underline">
                  ...
                  <td className="px-4 py-3"><StatusBadge status={v.status.annualFee} daysRemaining={daysRemaining(v.annualFeeExpiry)} /></td>
                  <td className="px-4 py-3"><StatusBadge status={v.status.insurance} daysRemaining={daysRemaining(v.insuranceExpiry)} /></td>
                  <td className="px-4 py-3"><StatusBadge status={v.status.roadworthiness} daysRemaining={daysRemaining(v.roadworthinessExpiry)} /></td>
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-1">
                      <Link to={`/vehicles/${v.id}`} className="rounded-lg p-2 text-gray-500 hover:bg-gray-100" title="View"><Eye size={16} /></Link>
                      {canEdit && <Link to={`/vehicles/${v.id}/edit`} className="rounded-lg p-2 text-gray-500 hover:bg-gray-100" title="Edit"><Pencil size={16} /></Link>}
                      {canDelete && (
      ...
      <div className="space-y-3 sm:hidden">
        {loading && <div className="card px-4 py-10 text-center text-gray-400">Loading vehicles…</div>}
        {!loading && items.length === 0 && <div className="card px-4 py-10 text-center text-gray-400">No vehicles match your search or filters.</div>}
        {!loading &&
          items.map((v) => (
            <div key={v.id} className="card p-4">
              <div className="flex items-start justify-between">
                <button onClick={() => navigate(`/vehicles/${v.id}`)} className="font-mono-reg text-base font-semibold text-navy-900">
                  {v.registrationNumber}
                </button>
                <div className="flex gap-1">
                  <Link to={`/vehicles/${v.id}`} className="rounded-lg p-1.5 text-gray-500"><Eye size={16} /></Link>
                  {canEdit && <Link to={`/vehicles/${v.id}/edit`} className="rounded-lg p-1.5 text-gray-500"><Pencil size={16} /></Link>}
                  {canDelete && (
                    <button onClick={() => setDeleteTarget(v)} className="rounded-lg p-1.5 text-gray-500"><Trash2 size={16} /></button>
                  )}
                </div>
              </div>
              <div className="mt-1 text-sm text-gray-600">{v.ownerFullName} · {v.categoryName}</div>
              <div className="mt-3 flex flex-wrap gap-1.5">
                <StatusBadge status={v.status.annualFee} />
                <StatusBadge status={v.status.insurance} />
                <StatusBadge status={v.status.roadworthiness} />
              </div>
            </div>
          ))}
      </div>
      ...
      <ConfirmDialog
        ...
        message={`This will permanently remove vehicle ${deleteTarget?.registrationNumber} from the Dhaftharu. This action cannot be undone.`}

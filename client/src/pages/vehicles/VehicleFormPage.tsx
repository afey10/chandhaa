...
const inputClass =
  "w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm outline-none focus:border-navy-600 focus:ring-2 focus:ring-navy-100";
...
export default function VehicleFormPage() {
  ...
  useEffect(() => {
    api.get("/categories/vehicle-categories").then((res) => setCategories(res.data.items));
  }, []);
    ...
    api
      ...
      .then((res) => {
        ...
        setForm({
          ...
          annualFeeExpiry: v.annualFeeExpiry,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (photo) fd.append("photograph", photo);
  ...
  if (loading) {
    return (
      <AppLayout>
        <div className="py-20 text-center text-gray-400">Loading vehicle…</div>
      </AppLayout>
    );
  }
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-6">
          <section className="card p-5">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field label="Make">
                <input value={form.make} onChange={(e) => update("make", e.target.value)} className={inputClass} />
              </Field>
              <Field label="Model">
                <input value={form.model} onChange={(e) => update("model", e.target.value)} className={inputClass} />
              </Field>
              <Field label="Year">
                <input type="number" value={form.year} onChange={(e) => update("year", e.target.value)} className={inputClass} />
              </Field>
              <Field label="Colour">
          ...
          <section className="card p-5">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field label="Engine Number">
                <input value={form.engineNumber} onChange={(e) => update("engineNumber", e.target.value)} className={inputClass} />
              </Field>
              <Field label="Chassis Number">
                <input value={form.chassisNumber} onChange={(e) => update("chassisNumber", e.target.value)} className={inputClass} />
              </Field>
              <div className="sm:col-span-2">
                <Field label="Remarks">
                  <textarea value={form.remarks} onChange={(e) => update("remarks", e.target.value)} rows={3} className={inputClass} />
                </Field>
              </div>
              <div className="sm:col-span-2">
                <label className="mb-1 block text-sm font-medium text-gray-700">Vehicle Photograph</label>
                {existingPhoto && !photo && (
                  <img src={`/${existingPhoto}`} alt="Current vehicle" className="mb-2 h-28 w-28 rounded-lg object-cover" />
                )}
                <label className="flex w-fit cursor-pointer items-center gap-2 rounded-lg border border-dashed border-gray-300 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50">
                  <Upload size={15} />
                  {photo ? photo.name : "Choose image (JPG, PNG, WEBP — max 5MB)"}
          ...
          <div className="flex justify-end gap-3">
            <button type="button" onClick={() => navigate(-1)} className="rounded-lg border border-gray-300 px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50">
              Cancel
            </button>
            <button
              type="submit"
              disabled={submitting}
              className="rounded-lg bg-navy-900 px-6 py-2.5 text-sm font-semibold text-white hover:bg-navy-800 disabled:opacity-60"
            >
